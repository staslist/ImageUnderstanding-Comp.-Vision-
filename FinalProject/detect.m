function [x,y,score] = detect(I,template,ndet)
    ...
    while ((detcount <= ndet) & (i <= length(ind)))
        ...
        %{
        disp('xpixel in detect.m:');
        disp(xpixel);
        disp('x in detect.m:');
        disp(x);
        disp('ypixel in detect.m:');
        disp(ypixel);
        disp('y in detect.m:');
        disp(y);
        disp('size(template, 1) in detect.m:');
        disp(size(template, 1));
        disp('size(template, 2) in detect.m:');
        disp(size(template, 2));
        %}
        
        cond_x = min(abs(xpixel - x)) < size(template, 1)*4;
        cond_y = min(abs(ypixel - y)) < size(template, 2)*4;
